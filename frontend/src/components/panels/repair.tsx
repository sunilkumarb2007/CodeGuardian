  ...
  return (
    <Card accent="blue">
      <PanelHeading
        ...
        caption="Structured engineering findings — observation, evidence, hypothesis, decision."
      />
      {!investigation ? (
...
function ReplayCard({ side, tone }: { side?: ReplaySide; tone: 'fail' | 'pass' }) {
      ...
      {!replay ? (
        ...
        <div className="px-7 py-7">
          <div className="grid gap-6 lg:grid-cols-2">
            <ReplayCard side={replay.original} tone="fail" />
            <ReplayCard side={replay.patched} tone="pass" />
          </div>
  ...
  return (
    <Card>
      <PanelHeading
        ...
        caption="A patch is only deliverable when every gate reported by the backend passes."
        right={
          total > 0 ? (
  ...
  return (
    <Card accent="lime" className="overflow-hidden">
      <div className="grid-bg animate-gridDrift px-7 py-10">
        ...
        <h3 className="display-md mt-3">Patch ready for review</h3>
  ...
  return (
    <Card accent="blue">
      <PanelHeading
        ...
        caption="Static safety checks run on the patch before anything is replayed."
        right={
          compatibility?.result ? (
            <span className="pill border-signal-blue/60 text-signal-blue">{compatibility.result}</span>
          ) : null
        }
      />
      {!compatibility || compatibility.checks.length === 0 ? (
        <EmptyState message="No compatibility checks reported" />
      ) : (
        <div className="px-7 py-7">
          <div className="grid gap-x-10 sm:grid-cols-2">
            {compatibility.checks.map((check) => (
              <KeyValue key={check.label} label={check.label} value={check.value} />
            ))}
          </div>
          {compatibility.checkedFiles.length > 0 ? (
            <div className="mt-6 flex flex-wrap gap-2">
              {compatibility.checkedFiles.map((file) => (
                <span key={file} className="pill border-ink-600 text-ink-300">
                  {file}
                </span>
              ))}
            </div>
